...
local _ticker = Class()
function _ticker:ctor(delay, clock)
    self.m_delay = delay
    self.m_total = 0
    self.m_clock = clock
end

function _ticker:check(dt)
    local total = self.m_total
    total = total + dt
    if total >= self.m_delay then
        total = total - self.m_delay
        __update(self.m_clock)
    end
    self.m_total = total
end

function _clock:close()
...
function _clock:AddTimer(ms, cb, repeat_exec, ...)
    ...
    if not self.timer then
        self.timer = _ticker.new(self.m_delay, self)
...
function TimerUpdate(dt)
    for _,ticker in pairs(timers) do
        ticker:check(dt)
    end
end